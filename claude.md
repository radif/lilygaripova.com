...
- **Flip-opportunities subscription lander launched (`/flip-opportunities/` + `/ru/flip-opportunities/`).** Investor-audience counterpart to the buyer-focused off-market lander. Same two-page funnel pattern (index + thanks), same brand system, same off-market visual hook (blurred deal-card stack), but each preview card carries underwriting language (`List · ARV · Rehab`) and badges (`Distressed`, `Fixer`, `Value-Add`) instead of the buyer-side `Pocket / Coming Soon / Off-Market` taxonomy. Form fields: name, email, phone (required) plus three optional qualifiers tuned to investor segmentation (acquisition budget select, investor experience select, preferred areas free-text). Form names `flip-opportunities-en` and `flip-opportunities-ru`. **Indexability: `noindex, nofollow`** by deliberate match to the existing lander default. Unlike home-value (where the main page meta-description has been promising "Free home valuation" since 2024 and demanded an indexable canonical surface), the flip-opportunities lander is built specifically for Meta-platform paid traffic with planned A/B tests of (a) lander variants and (b) ad-traffic routing between the standalone lander and the main site. Indexing would add organic-search noise to the A/B-test attribution. Russian copy keeps the English terms `flip`, `flip-объекты`, `Distressed`, `Fixer`, `Value-Add`, `Pocket listing`, `MLS`, `ARV`, `rehab`, `spread`, `underwriting`, `acquisition`, `cash investor`, `builder`, `ADU` inline rather than translating them into Cyrillic equivalents (matches the brand voice rule for Russian real-estate professional vocabulary). Russian dropdown label: "FLIP-ОБЪЕКТЫ". Nav wired into KNOWLEDGE / РЕСУРСЫ dropdown on main pages and FAQ pages in both languages, plus mobile flat nav and footer (per the standard new-lander pattern). No sitemap entry (sitemap is reserved for indexable canonical URLs).
- **Dedicated home-valuation lander launched (`/home-value/` + `/ru/home-value/`).** Two-page funnel per language (lander + thanks) modeled on the off-market design but with a CMA-preview card stack (estimated value range bar, comparable-sales context, timing-and-strategy note) instead of blurred-address listings. The seller-side hook is visually distinct from the buyer-side off-market hook while reusing the same brand system (cream background, Tenor Sans + Questrial / PT Serif + PT Sans, olive-green accent). Form fields: name, email, phone, property address. Form names `home-value-lander-en` and `home-value-lander-ru` (distinguishes the lander from the inline hero widget already on the main pages which use `home-value` and `home-value-ru`). **Indexable by deliberate exception to the lander default.** The main page's meta-description has been promising "Free home valuation available" since the December 2024 SEO baseline, so the lander is the canonical surface for that pitch and carries full SEO metadata, Service-schema JSON-LD, and hreflang reciprocity rather than `noindex,nofollow`. Added to `sitemap.xml` (2 new URLs with hreflang) and to the KNOWLEDGE / РЕСУРСЫ dropdown + mobile nav + footer on main pages and FAQ pages in both languages. Russian dropdown label: "ОЦЕНКА ДОМА". Form action redirects to `/home-value/thanks` (EN) / `/ru/home-value/thanks` (RU), each confirming a 1 to 2 business day turnaround for a personalized CMA. Service JSON-LD `Service` schema declares `priceRange: "$$"` and zero-cost `Offer` so AI engines can index "free home valuation" as a structured service rather than just marketing copy.
